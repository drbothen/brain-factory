...
**Changes in v0.4.20 (2026-05-18):**
- **TEST-ARCHITECTURE AMENDMENT (F-PHASE2-STEP-B-CLOSEOUT-O1 — per-hook bats model replaces consolidated hooks.bats):** CLAUDE.md states 4 times that every hook script has its own `plugins/brain-factory/tests/<hook-name>.bats` file (§HOOK TESTS, §TDD Inner Loop Discipline table, §Meta-Lint Contract Surface 2, §Build & Test header). The previous brief committed to "9 bats suites" with all 13 hooks consolidated into a single `hooks.bats` (established in F-PASS14-I1 v0.4.9 to preserve a suite-count-minimalism commitment). CLAUDE.md wins per Source-of-Truth Precedence. §Scope bats suites bullet updated to per-hook + category model: 8 category suites (`meta-lint.bats`, `skills.bats`, `templates.bats`, `policies.bats`, `adversary.bats`, `quarantine.bats`, `integration.bats`, `upgrade.bats`) plus 13 per-hook bats files at `tests/<hook-name>.bats` (one per hook script). No consolidated `hooks.bats` file. §References vsdd-factory comparison updated: "9 bats suites" → "8 category suites + 13 per-hook suites". §Scalability §2 and §v0.1 ship gate hook-performance / embedding_status paragraphs updated to reference the per-hook bats files. Self-Audit Checklist count claims updated accordingly. NFR-019 amended in separate file; BC-INDEX bumped. User-confirmed direction 2026-05-18. (F-PHASE2-STEP-B-CLOSEOUT-O1)
...
- **STRUCTURAL FIX (Post-convergence cleanup — gate-coverage extension + exclusion-list-extension protocol + audit-trail wording calibration):** The adversarial cascade reached streak 3/3 (CONVERGED) at Pass 22 on brief v0.4.14 (commit 7035315). This v0.4.15 burst closes three deferred non-blocking items before Phase 1b PRD work begins. (F-PASS20-S1, F-PASS20-O1, F-PASS21-S1)
  - **F-PASS20-S1 — Gate-coverage extension to SESSION-HANDOFF:** Extends the Self-Audit Checklist enforcement gate from a single-file grep on the product-brief to a two-file `for`-loop covering both the product-brief and the SESSION-HANDOFF. The SESSION-HANDOFF is the writer's working-memory document; Pass 19's fourth-level recursion demonstrated that handoff drift can re-prime the writer's vocabulary and reintroduce literal anchors in the brief on the very next fix-burst. The updated checklist item includes the two-file loop command and adds a NOTE explaining the exclusion-list-extension protocol (F-PASS21-S1 below).
  - **F-PASS21-S1 — Exclusion-list-extension protocol NOTE:** Adds commentary inside the Self-Audit Checklist gate item explaining that the gate's exclusion list (`WSL2`, the regex literal itself, and the listed legitimate L-prefixed words) is the authoritative set of acceptable matches; new domain tokens must be added to the exclusion list before introduction, not worked around by reverting the writing-technique principle. Protocol steps: (a) add to `grep -v` clause; (b) re-run gate; (c) record rationale in changelog.
  - **F-PASS20-O1 — Audit-trail wording calibration on three historical entries:** Softened absolute-immutability language in three Changelog structural-fix entries at v0.4.7, v0.4.10, and v0.4.11 that used absolute-elimination phrasing. The cascade's four-recursion record demonstrated these claims were optimistic as stated. Replaced absolute-elimination phrasings with scoped equivalents ("scope-eliminates … within this version's coverage" / "scope-eliminates … within this fix-burst's scope (re-verify in subsequent passes)") — preserving every audit-trail fact while removing the overstated future-immutability claim. No entry was rewritten; only the absolute-language phrases were adjusted.

**Changes in v0.4.14 (2026-05-15):**
- **STRUCTURAL FIX (Writing-technique principle — fourth-level recursion closure):** Pass 19 (F-PASS19-C1) caught the fourth-level recursion of the narrow-fix-with-broad-announcement pattern. The v0.4.13 Changelog entry described the v0.4.12 defect by quoting the literal line-number token inside backticks in its prose. The grep gate cannot distinguish a quoted literal from an active line-number citation — both produce a match. This is the fourth instance: Pass 16 caught the v0.4.8 incomplete sibling-sweep; Pass 17 caught the v0.4.11 overbroad coverage claim; Pass 18 caught the v0.4.12 fix-burst re-introducing a literal line-number anchor; Pass 19 catches the v0.4.13 Changelog entry itself containing a quoted literal. Local fix: replaced both occurrences of the backtick-quoted literal line-number token in the v0.4.13 Changelog bullet with semantic-anchor language — "literal line-number anchor" and "literal line-number anchor" in place of the token. Structural enforcement: (1) hardened the Self-Audit Checklist grep gate command to add a second exclusion for the regex pattern literal itself; (2) added a writing-technique NOTE to the Checklist item instructing that descriptions of this defect class must use semantic terms rather than quoting the token. This breaks the recursion at the writing layer, not only at the gate layer. (F-PASS19-C1 LOCAL + F-PASS19-C1 ENFORCEMENT)
- Hardened Self-Audit Checklist grep gate command: added `| grep -v 'L\[0-9\]+'` exclusion so the gate's own regex pattern literal cannot match itself if future refactors alter quoting style. (F-PASS19-S1)

**Changes in v0.4.13 (2026-05-15):**
- **STRUCTURAL FIX (Changelog audit-trail enforcement closure):** Pass 18 (F-PASS18-I1) caught a third-level recursion of the narrow-fix-with-broad-announcement pattern: the v0.4.12 Changelog entry contained a literal line-number anchor in its text (referencing the v0.4.11 coverage claim), directly replicating the defect class that v0.4.10's STRUCTURAL FIX claimed to eliminate. This is the third level of the same pattern — Pass 16 caught the v0.4.8 "at all callsites" incomplete sibling-sweep; Pass 17 caught the v0.4.11 "all structural-fix headings" incomplete coverage claim; Pass 18 catches the v0.4.12 fix-burst itself introducing a literal line-number anchor. Local fix: replaced the literal line-number anchor in the v0.4.12 entry (the bullet referencing the v0.4.11 coverage claim) with the semantic equivalent "the v0.4.11 'all structural-fix headings now use semantic labels' coverage claim". Structural enforcement: added a new Self-Audit Checklist item requiring a pre-commit grep gate returning zero matches before any version bump. This converts the v0.4.10 cultural claim to brief-level write-time enforcement. (F-PASS18-I1 LOCAL + F-PASS18-I1 ENFORCEMENT)
- Fixed miscounted environment claim in the v0.4.12 Changelog entry for F-PASS17-S2: "all five supported environments" → "all four currently-supported environments (macOS / Linux / Git-Bash / WSL2) plus the v1.0 native-Windows commitment" — v0.x supports four environments; native Windows is v1.0 future-state. (F-PASS18-S1)
- Flattened nested-parenthetical structure in Self-Audit Checklist item #1: "19 action templates = 15 author-committed + 4 community-optional" → "19 GH Action templates split 15 author-committed and 4 community-optional" — consistent with F-PASS17-S2 cross_platform flatten discipline applied in v0.4.12. (F-PASS18-O2)

**Changes in v0.4.12 (2026-05-15):**
- **STRUCTURAL FIX (Changelog completeness — v0.4.8 back-fill):** Promoted two v0.4.8 changelog bullets to `**STRUCTURAL FIX (semantic-label):**` form: (1) Citation shorthand sibling-sweep (F-PASS12-O1 / F-PASS13-O1) and (2) §Changelog notation cleanup (F-PASS12-O2 / F-PASS13-O2). These were structural-discipline work per SESSION-HANDOFF.md §5's v0.4.8 row but lacked the STRUCTURAL FIX heading. v0.4.11's claim "all structural-fix headings in the Changelog now use semantic labels" was overbroad — it was true for headings that existed but FALSE for completeness. This is the third instance of the "narrow-fix-with-broad-announcement" defect-pattern class (Pass 16 caught "at all callsites" incomplete; Pass 17 caught "all structural-fix headings" incomplete). Amended the v0.4.11 'all structural-fix headings now use semantic labels' coverage claim to reflect the back-fill and bar a recurrence (F-PASS17-I1).
...
- Flattened frontmatter `cross_platform` nested parentheticals: `macOS + Linux + (Windows via Git Bash or WSL2) (native Windows = v1.0)` → `macOS + Linux + Git-Bash + WSL2 (native Windows = v1.0)` — preserves all four currently-supported environments (macOS / Linux / Git-Bash / WSL2) plus the v1.0 native-Windows commitment while eliminating nested-parenthetical structure (F-PASS17-S2).
...
- **STRUCTURAL FIX (Per-version attestation collapse):** collapsed Self-Audit Checklist per-version annotations (v0.3.0 through v0.4.6) to single "See Changelog at top of brief" references. Scope-eliminates the per-version-attestation drift class within this version's coverage (F-PASS11-I1). Same drift-eliminate discipline applied in v0.4.5 (grep-anchors for L-number refs) and v0.4.6 (creation-date anchors for line counts).
...
**Commitment:** `/brain:lint-wiki` completes wikilink integrity checks via index-first lookup (O(n) scan of `index.md`, not O(n²) cross-product). Hook performance budget: every hook in the 13-hook set processes its sample payload in under 100ms p99 — asserted in per-hook latency test cases inside the per-hook bats file for that hook (`plugins/brain-factory/tests/<hook-name>.bats`) as a v0.1 ship gate. Each per-hook bats file is one of the 13 per-hook suites in the per-hook + category test model (see §Test architecture). Wikilink validation across a 500+ page wiki may require incremental design (Phase 1c architecture concern); the bats test budget covers single-payload performance, not full-wiki scan.
...
- **Hook performance budget test:** the v0.1 ship gate adds **explicit hook-performance test cases in each hook's per-hook bats file** (`plugins/brain-factory/tests/<hook-name>.bats`, one file per hook in the per-hook + category test model — see §Test architecture). Each per-hook bats file asserts that the hook processes its sample payload in under 100ms p99. **This is a new addition to the gate, supplementing phased-build-plan §5.11's exit criteria.** Wikilink validation across a 500+ page wiki may exceed this budget; that case is incremental-design (Phase 1c architecture concern), not a v0.1 gate criterion.
- **`validate-frontmatter-schema.sh` embedding_status enforcement test (new v0.1 ship gate addition).** The hook rejects wiki page writes lacking the `embedding_status` frontmatter field. Positive case: write a wiki page with `embedding_status: pending` — hook exits 0 (no advisory). Negative case: write a wiki page without `embedding_status` — hook exits 2 (block). Both cases asserted in `plugins/brain-factory/tests/validate-frontmatter-schema.bats` (the per-hook bats file for `validate-frontmatter-schema.sh`, within the per-hook + category test model — see §Test architecture). This supplements the hook performance bats test at the immediately-preceding item; together they validate both the v0.4.1 mandatory-field commitment (Scalability §6) AND the v0.1 hook-perf budget. **This is a new addition to the gate, supplementing phased-build-plan §5.11's exit criteria.**
...
- **Bats test suites: 8 category suites + 13 per-hook suites.** Category suites (8): `meta-lint.bats`, `skills.bats`, `templates.bats`, `policies.bats`, `adversary.bats`, `quarantine.bats`, `integration.bats`, `upgrade.bats`. Per-hook suites (13): one `tests/<hook-name>.bats` file per hook script in `plugins/brain-factory/hooks/` (e.g., `tests/validate-frontmatter-schema.bats`, `tests/quarantine-fetch.bats`, etc.). No consolidated `hooks.bats` file. `meta-lint.bats` asserts (a) all 8 named category suites exist and (b) every `hooks/*.sh` has a matching `tests/<hook-name>.bats` file. Per CLAUDE.md TDD Inner Loop Discipline, per-hook files enable `bats tests/<hook-name>.bats -f "<test name>"` targeted iteration. (`llm-second-brain-plugin-plan.md` §4; CLAUDE.md Meta-Lint Contract; CLAUDE.md §TDD Inner Loop Discipline)
...
- [x] Did I cross-check every `locked_decisions` field in frontmatter against the body section that implements it, asserting numeric, naming, and scope consistency? **Yes.** All `locked_decisions` fields verified against body sections: counts (26 skills, 14 agents, 13 hooks, 19 actions, 8 category suites + 13 per-hook bats suites, 8 wclaude absorptions), scale targets, reference repo layout, platform commitments, and stage_3_locks path all cross-checked and consistent. **Per-version fix-burst details: see the Changelog block at top of brief.**
- ☐ **Changelog audit-trail discipline (v0.4.10 STRUCTURAL FIX enforcement — extended to handoff):** Before committing any version bump, run:

  ```bash
  for f in .factory/specs/product-brief.md .factory/SESSION-HANDOFF.md; do
    grep -nE '\bL[0-9]+\b' "$f" | grep -v WSL2 | grep -v 'L\[0-9\]+' | grep -v 'LinkedIn\|License\|LTS\|Linux\|Lobster\|Lock\|Loom\|Loki'
  done
  ```
