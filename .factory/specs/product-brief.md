...
- **STRUCTURAL FIX (Changelog audit-trail enforcement closure):** Pass 18 (F-PASS18-I1) caught a third-level recursion of the narrow-fix-with-broad-announcement pattern: the v0.4.12 Changelog entry contained the literal anchor `L57` in its text (referencing the v0.4.11 coverage claim), directly replicating the defect class that v0.4.10's STRUCTURAL FIX claimed to eliminate. This is the third level of the same pattern — Pass 16 caught the v0.4.8 "at all callsites" incomplete sibling-sweep; Pass 17 caught the v0.4.11 "all structural-fix headings" incomplete coverage claim; Pass 18 catches the v0.4.12 fix-burst itself introducing a literal line-number anchor. Local fix: replaced the `L57` literal anchor in the v0.4.12 entry (the bullet referencing the v0.4.11 coverage claim) with the semantic equivalent "the v0.4.11 'all structural-fix headings now use semantic labels' coverage claim". Structural enforcement: added a new Self-Audit Checklist item requiring a pre-commit `grep -nE '\bL[0-9]+\b' .factory/specs/product-brief.md | grep -v WSL2` returning zero matches before any version bump. This converts the v0.4.10 cultural claim to brief-level write-time enforcement. (F-PASS18-I1 LOCAL + F-PASS18-I1 ENFORCEMENT)
