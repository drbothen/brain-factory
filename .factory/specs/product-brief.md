...
- ☐ **Changelog audit-trail discipline (v0.4.10 STRUCTURAL FIX enforcement):** Before committing any version bump, run `grep -nE '\bL[0-9]+\b' .factory/specs/product-brief.md | grep -v WSL2 | grep -v 'L\[0-9\]+'` and confirm it returns zero matches. The Changelog block must contain zero literal line-number anchors. Pass 18 (F-PASS18-I1) demonstrated that absent this enforcement, the v0.4.10 "permanent elimination" claim regresses on the very fix-burst that addresses the recurrence. Pass 19 (F-PASS19-C1) demonstrated a fourth-level recursion: the v0.4.13 changelog entry described the defect by quoting the literal token itself, which the grep gate cannot distinguish from an active line-number citation. NOTE: When describing this defect class in the Changelog or anywhere else, use semantic terms (e.g., "literal line-number anchor", "L-prefixed token") rather than quoting the literal `L<number>` token; the grep gate cannot distinguish quoted-literal from active citation. Use semantic anchors (e.g., "the v0.4.11 coverage claim", "the bullet immediately following") instead.
