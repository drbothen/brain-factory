...
bats (`tests/validate-publish-state.bats` + skills.bats with LinkedIn DTU mock):
...
skill, LinkedIn DTU mock, tests/validate-publish-state.bats, and skills.bats

## Changelog

### v1.1 (2026-05-18)

**STRUCTURAL FIX (F-PHASE2-STEP-B-CLOSEOUT-O1-CASCADE — §Verification Mechanism and §Status updated to per-hook .bats convention):** Mechanism changed from `bats (hooks.bats + skills.bats with LinkedIn DTU mock)` to `bats (tests/validate-publish-state.bats + skills.bats with LinkedIn DTU mock)`. §Status reference to "hooks.bats / skills.bats" updated to "tests/validate-publish-state.bats, and skills.bats". The hook tests for validate-publish-state.sh now live in the per-hook file `tests/validate-publish-state.bats`. Cascades from SS-18 v1.5 per-hook .bats reversal (F-PHASE2-STEP-B-CLOSEOUT-O1). [audit-trail]
