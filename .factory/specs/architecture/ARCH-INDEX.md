...
- [x] **Dual-scope discipline (F-PASS10-O1):** Every codified discipline must declare two scopes: (a) incremental scope — checked on every burst; (b) canonical-baseline scope — one-time sweep over entire spec inventory at codification time. Incremental-only disciplines allow pre-existing defect inventory to survive indefinitely. Incremental scope: when adding a new Self-Audit item, both scopes must be declared before the burst commits. Canonical-baseline scope: Pass 11 F-PASS11-C2 retroactively audited all six prior Self-Audit items added in Passes 5–10 — dual-scope declarations added to each item in this burst. The dual-scope discipline applies to itself: declaring it without applying it retroactively would be an incremental-only discipline. (Self-application confirmed F-PASS11-C2.)
  ...
  **Same-commit-sibling-check (F-PASS19-O1 closure):** Before committing any burst that includes a new discipline codification (including extensions of existing disciplines), the architect MUST verify that ALL discipline codifications in the burst — including the codification(s) motivating the burst itself — satisfy the canonical-baseline scope sweep coverage sub-rule. Self-check procedure: grep the burst's own diff for new `Canonical-baseline scope` clauses; for each, verify the clause enumerates inventory swept (e.g., "18 prior reports", "all 27 VPs", "all 64 architecture artifacts"), not just motivating findings without enumeration. Incremental scope: applied before commit of any burst containing new discipline codifications or extensions. Canonical-baseline scope: Pass 19 codification triggered by F-PASS18-S1/F-PASS18-O1 same-commit sibling violation (Pass 18 architect burst a73b64a codified F-PASS18-O1 requiring canonical-baseline sweep coverage and in the same commit wrote F-PASS18-S1 with "going-forward enforcement only" disclaimer — the violated discipline and the violating codification were sibling text blocks committed simultaneously). Going-forward enforcement on ALL architect bursts containing discipline codifications. Pass 19 burst self-check: all new `Canonical-baseline scope` clauses in this burst enumerate actual inventory swept (F-PASS18-S1 clause: 18 prior reports scanned, grep methods enumerated, 0 additional fabrications found; F-PASS18-O1 rephrasing clause: no new canonical-baseline inventory required — this is a text-accuracy correction, not a new inventory-scope claim; this sub-clause: cites the motivating incident and declares going-forward scope with Pass 19 self-check result stated inline). [audit-trail]
