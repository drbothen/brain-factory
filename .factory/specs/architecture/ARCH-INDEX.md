...
- [x] **Architecture artifact Changelog discipline (SS/ADR/VP) (F-PASS10-I2, tightened F-PASS12-I2, extended F-PASS13-C2):** For every `subsystems/SS-*.md`, `adr/ADR-*.md`, and `verification-properties/VP-*.md` file that has had any content edit past initial creation (regardless of version number), verify the file body contains a `## Changelog` section and the frontmatter version is past "1.0". Content edits without version bumps create audit-trail gaps. Bash sweep:
  ...
  **F-PASS16-O1 binding-scope adjudication:** The F-PASS14-C1 enumeration discipline binds at ALL levels — including ARCH-INDEX's own Changelog narratives, not only target-file (SS-NN/ADR/VP) Changelog narratives. Rationale: the discipline's purpose is to ensure Changelog entries faithfully record what actually changed, with one bullet per discrete modification and no invented attributions. This purpose applies equally whether the Changelog being written is in a VP file, an ADR file, or in ARCH-INDEX itself. ARCH-INDEX's own Changelog entries for each pass must enumerate touched items accurately (grep ARCH-INDEX itself first); summary-level paraphrases of sub-discipline corrections (e.g., "four VP bullets corrected" without per-VP enumeration) are permitted only where ARCH-INDEX's entry already cross-references the detailed per-file bullets recorded in the affected files' own Changelogs. When ARCH-INDEX IS the source of record (no sub-file has its own Changelog section), per-item enumeration is mandatory in the ARCH-INDEX entry itself. (Adjudicated F-PASS16-O1; binds incrementally from this burst forward.) [audit-trail]

  **F-PASS15-C1 clarification — Changelog amendments count as body modifications:** Amending a `## Changelog` section (struck framings, added bullets, corrected source-version citations, added Notes) is a body modification requiring version bump and new Changelog entry. The carve-out interpretation "Changelog reconstruction is completing v1.1 rather than creating new state" is rejected — every burst that modifies a Changelog body advances the version. ARCH-INDEX itself bumped v0.1.15 → v0.1.16 for F-PASS14-C1 Changelog corrections, setting the explicit precedent. This clarification binds the F-PASS13-C2 Architecture artifact Changelog discipline (incremental scope: before any architect burst that modifies an SS-NN, ADR, or VP body — including Changelog body — bump the version and add a Changelog entry before commit). Incremental scope: before any architect burst that modifies a Changelog section body in any SS-NN, ADR, or VP file, treat it as a body modification — bump the version and add a new Changelog entry before commit. Canonical-baseline scope: Pass 15 F-PASS15-C1 burst applied this clarification retroactively — 6 files (VP-014, VP-021, VP-026, VP-027, ADR-004, ADR-009) were bumped from v1.1 to v1.2 for Pass 14 Changelog amendments that had not been version-bumped. (Codified F-PASS15-C1; dual-scope declared F-PASS16-C1.) [audit-trail]

  **F-PASS15-I1 strengthening — derived-cell-count enumeration discipline:** When citing F-PASS10-C1/I1 (or any sibling-sweep discipline that aligns multiple derived cells) in a VP/ADR/SS Changelog, the bullet MUST enumerate the SPECIFIC cells that had drift per ARCH-INDEX v0.1.12 (or the relevant pass-N entry). Do not claim "all three derived cells aligned" unless the ARCH-INDEX entry explicitly states all three had drift; if the ARCH-INDEX entry records drift for "Document Map Purpose and VP-INDEX Summary Title cells" only, the bullet must say "two of three derived cells aligned" or enumerate the two specific cells. Cross-reference VP-INDEX "VPs already aligned" inventory if applicable. Directionality must be stated correctly: derived cells aligned TO the canonical VP file H1 (not "H1 aligned with cells"). Incremental scope: applied when writing any Changelog bullet that cites a sibling-sweep discipline affecting multiple derived cells — enumerate specific cells, not "all three." Canonical-baseline scope: Pass 15 F-PASS15-I1 burst corrected 4 VP Changelog bullets (VP-014, VP-021, VP-026, VP-027) — overclaiming narratives reframed to enumerate specific affected cells and correct directionality. (Codified F-PASS15-I1; dual-scope declared F-PASS16-C1.) [audit-trail]

  **F-PASS15-I2 strengthening — initial-creation content discipline:** When applying the F-PASS14-C1 enumeration discipline, first verify that a body modification PAST INITIAL CREATION actually occurred. Initial-creation content reflecting parent-document decisions (e.g., a VP authored at v0.1.1 reflecting SS-NN decisions captured at v0.1.2 — where SS-NN parent decisions predated the VP's creation) is NOT a "modification past creation" and does NOT require attribution. The enumeration discipline targets post-creation body modifications only. Applying "modification observed but ARCH-INDEX history insufficient to attribute" to initial-creation content is a misapplication of F-PASS14-C1. Incremental scope: when applying F-PASS14-C1 enumeration to any architecture artifact Changelog, first confirm that a post-creation body modification actually occurred before attributing; do not misapply F-PASS14-C1 to initial-creation content. Canonical-baseline scope: Pass 15 F-PASS15-I2 burst corrected VP-014 v1.1 Changelog — the F-PASS14-C1 Note attributing initial-creation content as "modification observed but ARCH-INDEX history insufficient to attribute" was removed; VP-014 initial-creation content (zero-argument CLI, E-INIT-002) was authored at VP creation and does not require post-creation attribution. (Codified F-PASS15-I2; dual-scope declared F-PASS16-C1.) [audit-trail]

  **F-PASS15-O1 bash sweep extension — timestamp-invariant check (`timestamp >= created`):** The bash sweep above (Architecture artifact Changelog discipline) was extended with a separate invariant check: `timestamp` must be `>=` `created` per the tri-partite semantic (F-PASS10-I3 / F-PASS11-C1). Prior bash sweep detected only Changelog presence vs absence; it did not verify the `timestamp >= created` invariant. New check added inside the loop: if `timestamp` is lexicographically before both `${created}T00:00:00` and `${created}`, emit "INVARIANT-FAIL". Catches creation-date inversion (e.g., timestamp 2026-05-14 on a file created 2026-05-15). Incremental scope: the bash sweep already runs before every architect burst touching SS-NN/ADR/VP files; the INVARIANT-FAIL check runs automatically as part of that sweep. Canonical-baseline scope: Pass 15 F-PASS15-O1 burst added the INVARIANT-FAIL check to the bash sweep; no files were found with `timestamp < created` at codification time (all 64 architecture artifacts verified clean). (Codified F-PASS15-O1; dual-scope declared F-PASS16-C1.) [audit-trail]
...
- [x] **Dual-scope discipline (F-PASS10-O1):** Every codified discipline must declare two scopes: (a) incremental scope — checked on every burst; (b) canonical-baseline scope — one-time sweep over entire spec inventory at codification time. Incremental-only disciplines allow pre-existing defect inventory to survive indefinitely. Incremental scope: when adding a new Self-Audit item, both scopes must be declared before the burst commits. Canonical-baseline scope: Pass 11 F-PASS11-C2 retroactively audited all six prior Self-Audit items added in Passes 5–10 — dual-scope declarations added to each item in this burst. The dual-scope discipline applies to itself: declaring it without applying it retroactively would be an incremental-only discipline. (Self-application confirmed F-PASS11-C2.)
  ...
  Canonical-baseline scope sweep coverage (F-PASS18-O1 closure): when codifying a new discipline, the architect MUST enumerate (in the Canonical-baseline scope clause) the full inventory swept at codification time, not just the findings that motivated the codification. The example list in the sub-rule body is authoritative for scope: the example header-text patterns IMPLY file classes by where such headers live (e.g., "(M confirmed disciplines)" implies operational state docs such as STATE.md and SESSION-HANDOFF §5 where discipline counts live; "N fix-bursts complete" implies STATE.md and SESSION-HANDOFF frontmatter where fix-burst counts live; "(N total items)" implies any document with item-count headers). When codifying a new discipline, the architect MUST enumerate (in the Canonical-baseline scope clause) the full inventory implied by the example patterns — not just the findings that motivated the codification. Incremental scope: applied before any discipline codification's Canonical-baseline scope clause is finalized. Canonical-baseline scope: Pass 18 codification triggered by discipline #23 example list ("(M confirmed disciplines)", "N fix-bursts complete") implying operational-state-doc scope when actual sweep covered only 2 motivating findings; F-PASS18-I2 closure separately performs the omitted sweep. (F-PASS19-I2 correction: original text claimed "if an example references a file class (e.g., 'operational state docs' or 'all SS/ADR/VP files')" — this misrepresented discipline #23's actual example list, which contains header-text patterns, not file-class references; rephrased to accurately describe the inference chain from header-text patterns to implied file classes.)

  **Same-commit-sibling-check (F-PASS19-O1 closure):** Before committing any burst that includes a new discipline codification (including extensions of existing disciplines), the architect MUST verify that ALL discipline codifications in the burst — including the codification(s) motivating the burst itself — satisfy the canonical-baseline scope sweep coverage sub-rule. Self-check procedure: grep the burst's own diff for new `Canonical-baseline scope` clauses; for each, verify the clause enumerates inventory swept (e.g., "18 prior reports", "all 27 VPs", "all 64 architecture artifacts"), not just motivating findings without enumeration. Incremental scope: applied before commit of any burst containing new discipline codifications or extensions. Canonical-baseline scope: Pass 20 canonical-baseline sweep of prior architect bursts containing discipline codifications (15 prior bursts per STATE.md cascade table; Passes 4–18 architect commits). Sweep methodology: identify bursts that introduced NEW discipline codifications post-F-PASS18-O1 codification (Pass 18 burst onward); audit each for same-commit sibling-violations within the burst. Pre-F-PASS18-O1 bursts (Passes 4–17 architects) are not retroactively in-scope per F-PASS10-O1 dual-scope discipline ("codified discipline binds incrementally from codification time forward") — sibling-check rule did not exist during those bursts. Post-F-PASS18-O1 bursts in scope: Pass 18 a73b64a and Pass 19 9172878. Sweep result: 2 same-commit sibling-violations identified — (1) Pass 18 a73b64a: F-PASS18-S1 canonical-baseline clause wrote "going-forward enforcement only" in the same commit that codified F-PASS18-O1 requiring canonical-baseline enumeration; caught by Pass 19 F-PASS19-C1; closed Pass 19 9172878. (2) Pass 19 9172878: F-PASS19-O1 codification exempted its own canonical-baseline scope clause via inline carve-out ("this sub-clause: cites the motivating incident and declares going-forward scope"), constituting a within-codification self-exemption; caught by Pass 20 F-PASS20-C1; closed this Pass 20 burst. Pass 19 burst self-check (corrected per F-PASS20-I2): all new `Canonical-baseline scope` clauses in that burst enumerate actual inventory swept (F-PASS18-S1 clause: 18 prior reports scanned, grep methods enumerated, 0 additional fabrications found; F-PASS18-O1 rephrasing clause: no new canonical-baseline inventory required — text-accuracy correction only); F-PASS19-O1 own clause did NOT enumerate inventory — this is the defect closed by this Pass 20 burst. Going-forward enforcement: all architect bursts after this commit MUST satisfy same-commit-sibling-check before commit. [audit-trail]
- [x] **Adversary pre-flight grep verification (F-PASS11-O1):** For any writing-technique recursion finding, the adversary MUST first run the five-file gate Clauses 1+2 against the alleged offending text. If zero matches, the meta-narration about prior violations is NOT itself a violation — demote the finding from CRITICAL/IMPORTANT to OBSERVATION-only, or strike the finding entirely. This prevents the adversary-misdiagnosis class observed in Pass 10 F-PASS10-C2 (false-positive: the v0.1.11 F-PASS9-I1 entry was already semantic-only when Pass 10 flagged it). Incremental scope: adversary applies this pre-flight before filing any writing-technique recursion finding. Canonical-baseline scope: one-time codification in this Pass 11 burst; no retroactive audit required since prior finding classification is historical. (Added F-PASS11-O1.)
  ...
  By extension (F-PASS18-S1): before citing factual evidence about file content (versions, frontmatter, Changelog entries, ranges, counts, etc.) in any finding's Evidence section, the adversary MUST grep-verify the cited fact against the file. Citing an unverified fact as evidence is a fabrication-class defect even if the finding's conclusion is correct. Incremental scope: applied before any adversary cites a factual claim about file content in any finding Evidence section. Canonical-baseline scope: Pass 19 F-PASS19-C1 architect burst performed systematic sweep of all 18 prior adversary pass reports (adversary-pass-1.md through adversary-pass-18.md) for fabricated factual evidence claims — grep methods: (a) `grep -hn "version:"` across all 18 reports for version-citation lines; (b) `grep -hn "has v[0-9]\.[0-9]\|at v[0-9]\.[0-9]\|is at v[0-9]\." ` for "X has/is/at vN.M" claims; (c) spot-verification of high-confidence factual claims against current and historically contemporaneous file state. Sweep result: 1 fabrication confirmed — Pass 17 F-PASS17-S1 evidence asserted "VP-014-brain-init-scaffold.md: v1.3, v1.2, v1.1" (actual VP-014 version at Pass 17 time: v1.2 only; v1.3 entry fabricated). 0 additional fabrications detected beyond F-PASS17-S1 across all 18 prior reports. Historical state claims (e.g., "VP-012 is at v1.3" in Pass 11 context, before Pass 13 VPs were at v1.0) verified as accurate for their contemporaneous pipeline position. [audit-trail]
- [x] **Changelog version-monotonicity check (F-PASS16-I1):** Changelog entries MUST appear in strict descending semver order — each `### vX.Y.Z` entry must be followed by `### vX.Y.(Z-1)` (or the next-lower version). No version may appear out of sequence (e.g., v0.1.12 between v0.1.15 and v0.1.14 is a monotonicity violation). Bash sweep:
  ...
  Incremental scope: when adding a new Changelog entry to any architecture artifact, PRD, supplement, or BC file, insert it at the TOP of the Changelog section (after `## Changelog`) — always newer version before older. Verify by running the bash sweep above before commit. Canonical-baseline scope: Pass 16 F-PASS16-I1 burst corrected ARCH-INDEX.md — the v0.1.12 entry had survived 15+ passes in the wrong position (between v0.1.15 and v0.1.14); moved to correct position between v0.1.13 and v0.1.11. All other architecture artifact Changelog sections verified monotone at codification time (verified per-file via the bash sweep above; SS-NN files range v1.0 through v1.4 (SS-02 at v1.2; SS-18 at v1.4; all others at v1.1); ADR files range v1.0 through v1.2 (ADR-003, ADR-006, ADR-010, ADR-012, ADR-013, ADR-016 at v1.1; ADR-004, ADR-009 at v1.2; all others at v1.0); VP files range v1.0 through v1.3 (VP-004 at v1.1; VP-014, VP-021, VP-026, VP-027 at v1.2; VP-012 at v1.3; all others at v1.0); no out-of-order violations found in any file). Pass 17 canonical-baseline sweep extended discipline scope to PRD/supplements/BC-INDEX/95 BCs; confirmed all currently monotone (PRD: 10 entries v0.1.0..v0.1.9; BC-INDEX: 9 entries v0.1.0..v0.1.8; VP-INDEX: 6 entries v0.1.0..v0.1.6; PRD supplements and 95 BC files have no Changelog sections and are safely skipped by the `grep -q "^### v"` gate). (Codified F-PASS16-I1; dual-scope declared in that burst; bash sweep extended to PRD/supplements/BC-INDEX/BCs and canonical-baseline rationale corrected F-PASS17-S1 + F-PASS17-I3(a).) [audit-trail]

- [x] **Header-vs-body count check (F-PASS17-I1 closure, F-PASS17-O1 process-gap):** For any section header that contains a count claim (e.g., "(N total items)" in any document, "(M confirmed disciplines)" in STATE.md and SESSION-HANDOFF §5, "N fix-bursts complete" in STATE.md and SESSION-HANDOFF frontmatter), verify the count matches the visible body item / row / list-entry count. Headers MUST accurately describe the body they introduce. Paper-fixing a header by updating the count claim without reconciling the body is a TD-VSDD-059 violation. (F-PASS19-I2 extension: example list extended from header-text patterns only to include explicit file-class anchors for each example, per F-PASS18-O1 canonical-baseline scope sweep coverage rephrasing — makes the implied file classes explicit rather than requiring inference.)
...
**STRUCTURAL FIX (F-PASS20-C1 — F-PASS19-O1 canonical-baseline scope clause replaced with actual 15-prior-burst sweep enumeration):** The F-PASS19-O1 Canonical-baseline scope clause previously cited only the motivating Pass 18 a73b64a incident and declared "Going-forward enforcement on ALL architect bursts containing discipline codifications" without enumerating prior bursts swept. This directly violated F-PASS18-O1 (which F-PASS19-O1 extends), constituting the 10th recurrence of the meta-rule self-violation class and the first within-codification self-exemption variant. Pass 20 architect burst performed the required sweep of prior architect bursts per STATE.md cascade table (15 prior bursts; Passes 4–18 architect commits). Sweep scope limited to post-F-PASS18-O1 bursts per F-PASS10-O1 dual-scope discipline (rule binds from codification forward; Passes 4–17 architects predate the rule and are not retroactively in scope). Post-F-PASS18-O1 bursts audited: Pass 18 a73b64a and Pass 19 9172878. Sweep result: 2 same-commit sibling-violations found — Pass 18 a73b64a (F-PASS18-S1; already documented and closed Pass 19 9172878) and Pass 19 9172878 (F-PASS19-O1 self-exemption; closed this burst). Canonical-baseline scope clause in the Self-Audit Checklist F-PASS19-O1 sub-rule replaced with full sweep enumeration. [audit-trail]

**STRUCTURAL FIX (F-PASS20-I2 — F-PASS19-O1 circular carve-out removed from inline self-check enumeration):** The F-PASS19-O1 sub-clause inline self-check enumeration contained a third bullet ("this sub-clause: cites the motivating incident and declares going-forward scope with Pass 19 self-check result stated inline") that exempted F-PASS19-O1's own codification via two mechanisms: (1) "cites the motivating incident" reframes the clause as historical narrative rather than new codification, and (2) "declares going-forward scope" relabels absent canonical-baseline coverage as legitimate scope-limiting. Both mechanisms are precisely what F-PASS18-O1 and F-PASS19-O1 were designed to block. Carve-out bullet removed. Self-check enumeration in both the Self-Audit Checklist sub-rule text and the v0.1.21 Changelog entry updated to accurately characterize the Pass 19 burst results: F-PASS18-S1 and F-PASS18-O1 rephrasing clauses passed the self-check; F-PASS19-O1's own clause did NOT pass (inline carve-out was the defect, caught by Pass 20 F-PASS20-C1, closed this burst). [audit-trail]

### v0.1.21 (2026-05-17)

**STRUCTURAL FIX (F-PASS19-C1 — F-PASS18-S1 canonical-baseline sweep performed; "going-forward enforcement only" disclaimer replaced with actual enumeration):** The F-PASS18-S1 Canonical-baseline scope clause previously read "no other historical fabrications enumerated as the prior-pass evidence review was not systematic — going-forward enforcement only." This directly violated F-PASS18-O1 (codified in the same commit a73b64a), constituting the 9th recurrence of the meta-rule self-violation class and the first same-commit sibling-violation variant. Pass 19 architect burst performed the required systematic sweep of all 18 prior adversary pass reports (adversary-pass-1.md through adversary-pass-18.md). Sweep method: (a) `grep -hn "version:"` across all 18 reports for version-citation lines; (b) `grep -hn "has v[0-9]\.[0-9]\|at v[0-9]\.[0-9]\|is at v[0-9]\." ` for version-state claims; (c) spot-verification of high-confidence factual claims against current and contemporaneous file state. Sweep result: 1 fabrication confirmed — Pass 17 F-PASS17-S1 evidence asserted "VP-014-brain-init-scaffold.md: v1.3, v1.2, v1.1" (actual VP-014 version at Pass 17 time: v1.2 only; v1.3 entry fabricated). 0 additional fabrications detected beyond the already-documented F-PASS17-S1 across all 18 prior reports. Canonical-baseline scope clause in F-PASS18-S1 text replaced with enumeration of sweep method and result. [audit-trail]

**STRUCTURAL FIX (F-PASS19-I2 — F-PASS18-O1 misanchored example-list claim fixed in both directions):** (a) F-PASS18-O1 extension text previously claimed "the example list in the sub-rule body is authoritative for scope: if an example references a file class (e.g., 'operational state docs' or 'all SS/ADR/VP files'), that class is in scope and the sweep must cover it." This misrepresented discipline #23's actual example list, which contains header-text patterns ("(N total items)", "(M confirmed disciplines)", "N fix-bursts complete"), not file-class references. Rephrased to accurately state that the header-text patterns IMPLY file classes by where such headers live, and to enumerate the implied file classes explicitly: "(M confirmed disciplines)" implies STATE.md and SESSION-HANDOFF §5; "N fix-bursts complete" implies STATE.md and SESSION-HANDOFF frontmatter; "(N total items)" implies any document with item-count headers. (b) Discipline #23 example list extended to include explicit file-class anchors alongside each example header-text pattern, making the implied file classes explicit rather than requiring inference. Both fixes applied in this burst. [audit-trail]

**NEW SUB-RULE (F-PASS19-O1 — same-commit-sibling-check sub-clause added to F-PASS18-O1 extension text under discipline #10):** Added "Same-commit-sibling-check" sub-clause: before committing any burst that includes a new discipline codification (including extensions), the architect MUST verify ALL discipline codifications in the burst — including the codification(s) motivating the burst itself — satisfy the canonical-baseline scope sweep coverage sub-rule. Self-check procedure: grep burst's own diff for new `Canonical-baseline scope` clauses; verify each enumerates inventory swept, not just motivating findings. Pass 19 burst self-check result (corrected per F-PASS20-I2 — original inline carve-out "this sub-clause: cites the motivating incident and declares going-forward scope" removed): F-PASS18-S1 canonical-baseline clause: 18 prior reports scanned, grep methods stated, 0 additional fabrications found — PASS. F-PASS18-O1 rephrasing clause: no new canonical-baseline inventory required (text-accuracy correction only) — PASS. F-PASS19-O1 own canonical-baseline scope clause: did NOT enumerate inventory at codification time (inline carve-out exempted it from its own rule); this defect caught by Pass 20 F-PASS20-C1 and closed in the Pass 20 burst by replacing the carve-out with the actual 2-sibling-violation sweep enumeration. [audit-trail]

### v0.1.20 (2026-05-17)

**STRUCTURAL FIX (F-PASS18-I1 closure — discipline #22 canonical-baseline rationale complete per-file enumeration):** Discipline #22 Canonical-baseline scope parentheticals were expanded from MAX-version-outlier-only to complete per-file enumeration. SS-NN parenthetical corrected from "(SS-18 at v1.4)" to "(SS-02 at v1.2; SS-18 at v1.4; all others at v1.1)". ADR parenthetical corrected from "(ADR-004 + ADR-009 at v1.2)" to "(ADR-003, ADR-006, ADR-010, ADR-012, ADR-013, ADR-016 at v1.1; ADR-004, ADR-009 at v1.2; all others at v1.0)". VP parenthetical corrected from "(VP-012 at v1.3)" to "(VP-004 at v1.1; VP-014, VP-021, VP-026, VP-027 at v1.2; VP-012 at v1.3; all others at v1.0)". All three enumerations verified against actual file frontmatter before commit (grep-confirmed). [audit-trail]

**STRUCTURAL FIX (F-PASS18-S1 closure — F-PASS11-O1 adversary pre-flight extended to factual-evidence cites):** F-PASS11-O1 adversary pre-flight verification discipline scope extended from "writing-technique recursion findings only" to "any factual evidence cite in any finding Evidence section." Extension text appended to the Self-Audit Checklist F-PASS11-O1 sub-rule: before citing factual evidence about file content (versions, frontmatter, Changelog entries, ranges, counts, etc.), the adversary MUST grep-verify the cited fact against the file. Citing an unverified fact as evidence is a fabrication-class defect even if the finding's conclusion is correct. Incremental scope declared: applied before any adversary cites a factual claim about file content in any finding Evidence section. Canonical-baseline scope declared: Pass 18 codification triggered by F-PASS17-S1 adversary-fabricated "VP-014 has v1.3" evidence (actual: v1.2); going-forward enforcement only. [audit-trail]

**STRUCTURAL FIX (F-PASS18-O1 closure — discipline #10 extended with canonical-baseline scope sweep coverage sub-item):** Discipline #10 (Dual-scope discipline) extended with canonical-baseline scope sweep coverage sub-item: when codifying a new discipline, the architect MUST enumerate (in the Canonical-baseline scope clause) the full inventory swept at codification time, not just motivating findings. The example list in the sub-rule body is authoritative for scope. Incremental scope declared: applied before any discipline codification's Canonical-baseline scope clause is finalized. Canonical-baseline scope declared: Pass 18 codification triggered by discipline #23 example list ("(M confirmed disciplines)", "N fix-bursts complete") implying operational-state-doc scope when actual sweep covered only 2 motivating findings; F-PASS18-I2 closure separately performs the omitted sweep. [audit-trail]
...
**STRUCTURAL FIX (F-PASS16-I1 — discipline #22 Changelog version-monotonicity check codified):** New Self-Audit Checklist item added: Changelog entries MUST appear in strict descending semver order. Bash sweep codified: `grep -nE '^### v' "$f" | awk '{print $2}' | sort -rV -c` exits 0 if entries are strictly descending. Scope: applies to ARCH-INDEX, VP-INDEX, and all SS-NN/ADR/VP files with Changelog sections. Dual-scope declared in the new checklist item. F-PASS16-O2 resolved by this codification. [audit-trail]

**STRUCTURAL FIX (F-PASS16-O1 — F-PASS14-C1 binding-scope adjudication):** Adjudication added inline to the F-PASS14-C1 Changelog reconstruction enumeration discipline sub-rule: the discipline binds at ALL levels including ARCH-INDEX's own Changelog narratives (not only target-file narratives). Rationale documented in the new F-PASS16-O1 annotation. ARCH-INDEX is the source of record when no sub-file has its own Changelog section; per-item enumeration is mandatory in the ARCH-INDEX entry itself in those cases. [audit-trail]
...
**STRUCTURAL FIX (F-PASS13-C1 — count-balance correction):** The timestamp freshness check Self-Audit item incorrectly claimed "34 bumped / 28 retained" for the Pass 11 canonical-baseline architecture sweep. Arithmetic: 34 + 28 = 62, but the actual artifact count is 64. Correct count: 34 bumped + 30 retained = 64. Self-Audit item corrected. New Self-Audit checklist item "Count balance check" codified: for any count claim in a canonical-baseline-scope clause, verify N + M = the total artifact count cited in the same clause before commit. [audit-trail]

**STRUCTURAL FIX (F-PASS13-C2 — architecture artifact Changelog discipline extended to ADR and VP scope):** The Pass 12 SS-NN Changelog discipline covered only `subsystems/SS-*.md` files. The same defect class (content-edit-history with no Changelog section, stuck at version "1.0") was present in 8 ADRs (ADR-003, ADR-004, ADR-006, ADR-009, ADR-010, ADR-012, ADR-013, ADR-016) and 5 VPs (VP-004, VP-014, VP-021, VP-026, VP-027). All 13 files back-filled to v1.1 with Changelog sections reconstructed from ARCH-INDEX history. Self-Audit item renamed from "SS-NN Changelog discipline" to "Architecture artifact Changelog discipline (SS/ADR/VP)". Bash sweep updated: trigger is now `timestamp > created` (content-edit detection) rather than the prior `version != 1.0` guard; scope expanded to all three artifact types. (8 ADRs + 5 VPs = 13 files modified in this burst, in addition to ARCH-INDEX itself.) [audit-trail]

**STRUCTURAL FIX (F-PASS13-I2 — stale PO follow-up instruction replaced with closure narrative):** The Timestamp Field Convention Policy section contained an operational instruction "PRD and BC-INDEX sweep: PO follow-up burst required..." that was already closed by PO burst ecbe056 (F-PASS12-C2). Stale instruction replaced with closure narrative: total canonical-baseline sweep is 134 files bumped + 31 files retained = 165 files in scope. [audit-trail]

**STRUCTURAL FIX (F-PASS13-I3 — F-PASS11-C2 / F-PASS11-I2 credit-drift reconciliation):** Both the F-PASS11-C2 and F-PASS11-I2 changelog entries claimed ownership of the same SS-NN Changelog discipline dual-scope amendment. Adjudication: F-PASS11-I2 is the sole owner (the finding is specifically about the SS-NN Changelog discipline dual-scope). F-PASS11-C2 list corrected from six items to five — item "(3) SS-NN Changelog discipline" removed. Corrective NOTE added to F-PASS11-C2 entry. [audit-trail]

### v0.1.14 (2026-05-16)

**STRUCTURAL FIX (F-PASS12-C1 — SS-NN timestamp self-violation resolution + count correction):** Pass 11 canonical-baseline sweep correctly identified that all 18 SS-NN files had content edits past initial creation (all are Case A), but the Self-Audit Checklist count claim of "26 ADRs/SS/VPs bumped" was wrong — actual count confirmed as 34. Investigation of ARCH-INDEX changelog entries v0.1.2 through v0.1.5 identified documented content edits for every SS-NN file: SS-01 (F-PASS1-I2/I3), SS-02 (F-PASS2-I4/I5 — already at v1.2), SS-03 (F-PASS2-I4), SS-04 (F-PASS1-C4/I12, F-PASS4-C2), SS-05 (F-PASS2-I4), SS-06 (F-PASS2-I4, F-PASS4-C2), SS-07 (F-PASS4-C2), SS-08 (F-PASS1-I6, F-PASS4-C2), SS-09 (F-PASS1-I5, F-PASS4-C2), SS-10 (F-PASS4-C2), SS-11 (F-PASS4-C2), SS-12 (F-PASS4-C2), SS-13 (F-PASS1-C7, F-PASS3-S2, F-PASS4-C2), SS-14 (F-PASS4-C2), SS-15 (F-PASS4-C2), SS-16 (F-PASS1-C5, F-PASS4-C2), SS-17 (F-PASS1-I4, F-PASS4-C2), SS-18 (multiple — already at v1.4). All 16 SS-NN files previously at v1.0 bumped to v1.1 with in-file Changelog sections added. SS-02 and SS-18 already conformant. No timestamp changes — all 18 SS-NN remain at 2026-05-16T00:00:00 (correct for Case A). Self-Audit Checklist count corrected from "26 bumped / 36 retained" to "34 bumped / 28 retained" (NOTE — F-PASS13-C1 corrects the "28 retained" claim to "30 retained" in v0.1.15; the arithmetic was 34 + 28 = 62, not 64). [audit-trail]
...
**STRUCTURAL FIX (F-PASS11-C2 — retroactive dual-scope audit of prior Self-Audit Checklist items):** Dual-scope declarations added to all prior Self-Audit Checklist items (passes 5–10) that codified a discipline but did not explicitly declare incremental vs. canonical-baseline scope. Items amended: (1) last_updated freshness check (added F-PASS5); (2) timestamp freshness check (F-PASS10-I3); (3) VP title canonical-baseline sweep (F-PASS10-C1/I1); (4) dual-scope discipline (F-PASS10-O1, self-applied); (5) adversary pre-flight grep verification (F-PASS11-O1). This is self-application of the F-PASS10-O1 dual-scope discipline to the checklist that carries the discipline itself. [audit-trail]

**NOTE (F-PASS12-I1 corrective — narrative-vs-content drift):** The original F-PASS11-C2 entry listed item names ("Pass 5 wikilink-resolution consistency item", "Pass 6 writing-technique five-file gate item", "Pass 7 architectural-constraints coverage item", "Pass 8 VP completeness item") that do not correspond to any actual Self-Audit Checklist items. The actual five items amended are enumerated above. This is the same hallucination-in-changelog class as F-PASS11-I1 (paper-fix retraction). Entry rewritten in-place with corrected item enumeration; the original item names were invention, not audit trail. [audit-trail]

**NOTE (F-PASS13-I3 credit reconciliation):** F-PASS11-I2 owns the dual-scope declaration for the SS-NN Changelog discipline (see F-PASS11-I2 entry below). The original F-PASS11-C2 entry incorrectly listed that discipline as item (3) in its amended list. Removed from F-PASS11-C2 to eliminate the dual-credit between F-PASS11-C2 and F-PASS11-I2 for the same single amendment. The corrected F-PASS11-C2 list is now five items (not six). F-PASS11-I2 is the sole owner of the Changelog discipline dual-scope declaration. [audit-trail]
