...
**AC-004** — After installation, invoking `skills/brain-health/run.sh` on a non-brain
directory emits an E-HEALTH-001 error envelope to stdout and exits 2 — a clean structured
response, not a raw bash crash. The verification property is: no unhandled bash errors,
no raw stack traces, structured JSON envelope always emitted on stdout.
(traces to BC-2.14.001 postcondition 3; VP-024 health-callable test; BC-2.01.006 EC-002)
...
| `brain-health/run.sh` on non-brain dir | Exit 2 with E-HEALTH-001 JSON envelope (no raw stack trace) | happy-path | BC-2.14.001 postcondition 3 |
...
| Post-upgrade health check | Exit 0 (GREEN or YELLOW acceptable per BC-2.01.006 postcondition 1) | integration | BC-2.14.002 postconditions 1–2 |
