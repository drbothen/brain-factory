...
- STORY-002 → STORY-004 [frontmatter-confirmed] (STORY-004 reuses the `tests/helpers.bash` pattern from STORY-002; STORY-004 uses standalone `brain-health-skill.bats` for skill tests and `brain-health-check.bats` for hook tests, not an extension of `tests/integration.bats`)
...
- STORY-003 → STORY-004 [frontmatter-confirmed] (STORY-004 uses `local-dev-test.sh` patterns established by STORY-003; STORY-004 delivers standalone `brain-health-skill.bats` and `brain-health-check.bats`, not additions to `tests/integration.bats`)
