...
**AC-004** — The test uses `SECONDS` bash builtin or `$EPOCHREALTIME` (bash 5.0+) for
timing. Do NOT use `date +%s%N` — macOS `date` does not support `%N` and silently
produces wrong output. Network fetch is bypassed by substituting the Defuddle call with
a `cat` of the fixture file for measurement purposes (test-mode flag or environment
variable `BRAIN_TEST_SKIP_FETCH=1`).
...
- The latency harness must NOT import Node.js timing libraries. Use `SECONDS` builtin or
  `$EPOCHREALTIME` (bash 5.0+) for timing. `date +%s%N` is forbidden — macOS does not
  support `%N`; use `date +%s` if a `date` call is needed.
...
| `date` | GNU/POSIX — use `date +%s` (second-resolution, POSIX-safe); see portability note | Wall-clock timing |
| `SECONDS` | bash builtin | Second-resolution timing (preferred over `date +%s%N`) |
| `$EPOCHREALTIME` | bash 5.0+ builtin | Microsecond-resolution alternative to `date +%s%N` |
| `bats-core` | 1.10+ (latest: 1.13.0) | CLAUDE.md §Build & Test |
| `jq` | 1.7+ (latest: 1.8.1) | Fixture manifest generation / validation |
| `shellcheck` | 0.10+ (latest: 0.11.0) | CLAUDE.md §Conventions |
| `shfmt` | 3.7+ (latest: 3.13.1) | CLAUDE.md §Conventions |

> **`date +%s%N` portability:** macOS `date` does NOT support `%N` (nanoseconds). For timing in this story, use `$SECONDS` (bash builtin, second-resolution) or `$EPOCHREALTIME` (bash 5.0+, microsecond-resolution) instead. If `date` is used, use `date +%s` (POSIX-safe, second resolution). The ratio assertion T(10K)/T(1K) operates on integer seconds, so second-resolution is sufficient.
