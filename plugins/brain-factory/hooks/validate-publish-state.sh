...
trap 'echo "Publish state hook blocked: internal error." >&2; exit 2' ERR
...
if [ ! -f "$HELPER" ]; then
  ...
  echo "Publish state hook blocked: internal error." >&2
  exit 2
fi
...
if [[ -z "$raw_stdin" ]]; then
  ...
  echo "Publish state hook blocked: malformed or empty hook payload." >&2
  exit 2
fi
...
if [[ ! -r "$file_path" ]]; then
  jq -cn \
    ...
    --arg trace "${HOOK_TRACE_ID}" \
    '{"continue":false,"decision":"block","reason":$msg,"hookSpecificOutput":{"hookEventName":"PostToolUse","code":$code,"trace":$trace}}'
  echo "Publish state hook blocked: cannot read file at ${file_path}." >&2
  exit 2
fi
...
if [[ -z "$new_status" ]]; then
  jq -cn \
    ...
    --arg trace "${HOOK_TRACE_ID}" \
    '{"continue":false,"decision":"block","reason":$msg,"hookSpecificOutput":{"hookEventName":"PostToolUse","code":$code,"trace":$trace}}'
  echo "Publish state hook blocked: missing status field in content file." >&2
  exit 2
fi
...
if [[ -z "$prior_status" ]]; then
  ...
  if [[ "$new_status" != "draft" ]]; then
    ...
    echo "Publish state hook blocked: new content files must start as draft." >&2
    exit 2
  fi
...
if [[ "$transition_valid" != "true" ]]; then
  ...
  jq -cn \
    ...
    '{"continue":false,"decision":"block","reason":$reason,
      "hookSpecificOutput":{"hookEventName":"PostToolUse","code":"E-PUBLISH-001","trace":$trace,"from_state":$from,"to_state":$to}}'
  echo "Publish state hook blocked: invalid state transition: ${prior_status} -> ${new_status}." >&2
  exit 2
fi
