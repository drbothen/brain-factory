...
2. **Read manifest and run duplicate guard.** Run `jq -r --arg url "$URL" '.sources[] | select(.url == $url) | .source_id' "${BRAIN_DIR}/.brain/manifest.json"`. If a slug is returned, exit with E-INGEST-001: "URL already ingested as <slug>. Sources are immutable."

3. **Fetch and clean with Defuddle.** Run `node "${CLAUDE_PLUGIN_ROOT}/scripts/defuddle-fetch.mjs" "$URL"`. On non-zero exit, propagate the error (E-INGEST-002 for non-200 HTTP, E-INGEST-003 for empty content). On success, capture cleaned markdown from stdout.
...
5. **Write source file.** Create `${BRAIN_DIR}/sources/${TOPIC}/${SLUG}.md` with YAML frontmatter: `title`, `url`, `ingested_at` (ISO 8601 UTC), `source_id` (the slug), `topic`, `embedding_status: pending`. Append Defuddle output as the body.
...
- All 50 tests in `plugins/brain-factory/tests/ingest-url.bats` pass.
